...
function nala_mxchat_signup_personal_context(string $normalized): array
{
    ...
    return [
        ...
        'woman' => nala_mxchat_signup_contains_any($normalized, [
            'woman', 'female', 'girl', 'lady', 'mom', 'mother', 'mujer',
            'mujeres', 'femenino', 'madre'
        ]),
        ...
        'physical_concern' => nala_mxchat_signup_contains_any($normalized, [
            'physical', 'strength', 'back pain', 'bad back', 'arthritis', 'disabled', 'disability',
...
function nala_mxchat_signup_personalized_context_line(array $context, string $lang, string $topic = 'general'): string
{
    if ($lang === 'es') {
        if (!empty($context['older'])) {
            return 'Para ti, la ventaja es aprender sin apuro, repetir lecciones y usar tu experiencia de vida para dar confianza, paciencia y profesionalismo al cliente.';
        }
        ...
        if (!empty($context['woman'])) {
            return 'La cerrajeria premia habilidad, criterio, confianza y trato profesional, asi que puede ser una muy buena opcion para mujeres que buscan una habilidad practica.';
        }
        ...
        if (!empty($context['busy'])) {
            return 'Si tu horario esta ocupado, el formato a tu propio ritmo te deja entrenar en sesiones cortas y repetir lecciones en los momentos que encajen contigo.';
        }
        ...
        if (!empty($context['price_sensitive']) && $topic !== 'refund') {
            return 'Si el presupuesto importa, empieza gratis primero y actualiza despues de comprobar que el programa encaja contigo.';
        }
        if (!empty($context['job_goal']) && $topic === 'placement') {
            return 'Para tu meta laboral, piensa en NALA como preparacion estructurada: habilidades, practica y certificacion para acercarte a mejores oportunidades.';
        }
        if (!empty($context['job_goal']) && $topic !== 'placement') {
            return 'Si tu meta es mejorar oportunidades de trabajo, enfocate en completar habilidades practicas y certificacion; NALA te da una base seria para presentarte con mas confianza.';
        }
        if (!empty($context['online_concern'])) {
            return 'Si te preguntas si online funciona, la ventaja es pausar, repetir y practicar a tu ritmo en vez de depender de una sola clase en vivo.';
        }
        if (!empty($context['physical_concern'])) {
            return 'Si te preocupa la parte fisica, empieza desde casa y luego elige los tipos de servicios de cerrajeria que mejor se adapten a ti.';
        }
    ...
    if (!empty($context['older'])) {
        return 'For you, the advantage is learning without rushing, replaying lessons, and using your life experience to bring patience, judgment, and professionalism to customers.';
    }
    if (!empty($context['young'])) {
    ...
    if (!empty($context['woman'])) {
        return 'Locksmithing rewards skill, judgment, reliability, and customer trust, so it can be a strong fit for women who want a practical trade path.';
    }
    if (!empty($context['career_change'])) {
    ...
    if (!empty($context['busy'])) {
        return 'If your schedule is busy, the self-paced format lets you train in short sessions and replay lessons when they fit your day.';
    }
    if (!empty($context['business_goal']) && $topic === 'bib') {
    ...
    if (!empty($context['price_sensitive']) && $topic !== 'refund') {
        return 'If budget is the concern, start free first and upgrade after you see the program fits you.';
    }
    if (!empty($context['job_goal']) && $topic === 'placement') {
        return 'For your work goal, think of NALA as structured preparation: skills, practice, and certification that move you toward better opportunities.';
    }
    if (!empty($context['job_goal']) && $topic !== 'placement') {
        return 'If your goal is better work opportunities, focus on completing practical skills and certification; NALA gives you a stronger foundation to present yourself with confidence.';
    }
    if (!empty($context['online_concern'])) {
        return 'If you are wondering whether online training works, the advantage is pausing, replaying, and practicing at your pace instead of relying on one live class.';
    }
    if (!empty($context['physical_concern'])) {
        return 'If physical comfort is a concern, start from home and then choose the locksmith services that fit you best.';
    }
    if (!empty($context['local_rules']) && $topic !== 'terms') {
...
function nala_mxchat_signup_fast_answer(string $message): string
{
    ...
    $asks_earnings = nala_mxchat_signup_contains_any($normalized, [
        ...
        'how much do locksmiths make', 'how much does a locksmith make',
        'what can i make', 'what could i make', 'what will i make',
        'how much can i earn', 'how much could i earn', 'what can i earn',
        'earn as a locksmith', 'earnings as a locksmith',
        'earning potential', 'income potential', 'income', 'make money',
        'salary', 'locksmith salary', 'locksmith pay', 'pay as a locksmith', 'average pay',
    ...
    $asks_license = nala_mxchat_signup_contains_any($normalized, [
        'license', 'licensing', 'licensed', 'state requirement', 'city requirement',
        'do i need a license', 'legal to work', 'licencia', 'licencias',
        'requisitos del estado', 'requisitos locales'
    ]);
    $asks_criminal = nala_mxchat_signup_contains_any($normalized, [
        'criminal record', 'felony', 'misdemeanor', 'background check',
        'record', 'conviction', 'antecedentes', 'delito', 'felonia'
    ]);
    $asks_online_hands_on = nala_mxchat_signup_contains_any($normalized, [
        'online only', 'online course', 'learn online', 'from home',
        'hands on', 'hands-on', 'practical practice', 'practice at home',
        'in person', 'presencial', 'en linea', 'desde casa', 'practica'
    ]);
    $asks_trust = nala_mxchat_signup_contains_any($normalized, [
        'is this legit', 'legit', 'scam', 'real school', 'is nala real',
        'can i trust', 'trustworthy', 'is this real', 'estafa', 'confiable',
        'es real'
    ]);
    $asks_woman_fit = nala_mxchat_signup_contains_any($normalized, [
        'as a woman', 'woman locksmith', 'female locksmith', 'for women',
        'can women', 'can a woman', 'mujer cerrajera', 'para mujeres'
    ]);
    $asks_beginner_fit = !empty($personal_context['beginner']) && nala_mxchat_signup_contains_any($normalized, [
        'can i learn', 'is it for me', 'will i be able', 'can i do this',
        'is this good for beginners', 'puedo aprender', 'es para mi',
        'principiantes'
    ]);
    $asks_business_launch = nala_mxchat_signup_contains_any($normalized, [
        'start a locksmith business', 'start my locksmith business',
        'start my own locksmith business',
        'open a locksmith business', 'launch a locksmith business',
        'start my own business', 'run my own business', 'get customers',
        'empezar un negocio de cerrajeria', 'abrir un negocio de cerrajeria'
    ]);
    $asks_job_guarantee = nala_mxchat_signup_contains_any($normalized, [
        'guarantee a job', 'guaranteed job', 'promise a job',
        'guarantee employment', 'guarantee income', 'guaranteed income',
        'garantizan trabajo', 'garantia de trabajo', 'garantizan ingresos'
    ]);
    ...
    if ($asks_price_objection) {
        if ($lang === 'es') {
            return 'Lo entiendo. Si el dinero esta apretado, empieza gratis hoy y prueba el entrenamiento antes de comprar. Premium sigue teniendo sentido porque no compras solo informacion; construyes una habilidad practica para trabajo pagado de cerrajeria.' . "\n\n" . 'Unos pocos servicios reales son el camino para ayudar a recuperar la inversion con el tiempo.' . nala_mxchat_signup_klarna_payment_phrase($lang) . "\n\n" . 'Empieza aqui: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'I understand. If money is tight, start free today and test the training before buying. Premium still makes sense because you are not just buying information; you are building a practical skill for paid locksmith work.' . "\n\n" . 'A few real jobs are the path that helps the training pay for itself over time.' . nala_mxchat_signup_klarna_payment_phrase($lang) . "\n\n" . 'Start here: [Start free lessons / Register](' . $register_link . ').';
    }

    if ($asks_think_or_family) {
        if ($lang === 'es') {
            return 'Claro, pensarlo esta bien, pero no pauses el primer paso. Empieza con las lecciones gratis hoy, compartelo con tu familia si quieres y deja que el entrenamiento muestre su valor.' . "\n\n" . 'Cuando estes listo, Premium te da el camino completo, certificacion y kit.' . $personal('decision') . nala_mxchat_signup_klarna_payment_phrase($lang) . "\n\n" . 'Comparte o empieza aqui: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'Of course, thinking it over is fine, but do not pause the first step. Start the free lessons today, share it with family if you want, and let the training show its value.' . "\n\n" . 'When you are ready, Premium gives you the full path, certification, and kit.' . $personal('decision') . nala_mxchat_signup_klarna_payment_phrase($lang) . "\n\n" . 'Share or start here: [Start free lessons / Register](' . $register_link . ').';
    }

    if ($asks_success) {
        $asks_pass_difficulty = nala_mxchat_signup_contains_any($normalized, [
            'hard to pass', 'is it hard', 'too hard', 'difficult to pass',
            'hard course', 'course hard', 'is the course hard',
            'dificil', 'muy dificil'
        ]);
        if ($lang === 'es') {
            if ($asks_pass_difficulty) {
                return 'No esta pensado para ser imposible; esta pensado para que avances con practica constante. NALA ayuda porque puedes repetir lecciones, estudiar a tu ritmo y construir confianza antes de los examenes.' . "\n\n" . 'Si tomas el entrenamiento en serio, este es un camino practico para convertir interes en una habilidad real.' . $personal('success') . "\n\n" . 'Empieza gratis y comprueba tu ritmo: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
            }
            return 'Si, si eres constante y practicas. NALA esta disenado para convertir motivacion en habilidades reales: lecciones a tu ritmo, contenido repetible, examenes y practica.' . "\n\n" . 'Este es exactamente el tipo de habilidad practica que abre oportunidades cuando la trabajas en serio.' . $personal('success') . "\n\n" . 'Empieza gratis y comprueba tu ritmo: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        if ($asks_pass_difficulty) {
            return 'It is not meant to be impossible; it is meant to reward steady practice. NALA helps because you can replay lessons, study at your own pace, and build confidence before the exams.' . "\n\n" . 'If you take the training seriously, this is a practical path for turning interest into a real skill.' . $personal('success') . "\n\n" . 'Start free and see your pace: [Start free lessons / Register](' . $register_link . ').';
        }

        return 'Yes, if you stay consistent and practice. NALA is built to turn motivation into real skills: self-paced lessons, replayable training, exams, and practical work.' . "\n\n" . 'This is exactly the kind of practical skill that opens opportunities when you work it seriously.' . $personal('success') . "\n\n" . 'Start free and see your pace: [Start free lessons / Register](' . $register_link . ').';
    }

    if ($asks_age_objection) {
        $age = $personal_context['age'] ?? null;
        if ($lang === 'es') {
            $age_intro = $age ? $age . ' es una muy buena edad' : 'una edad madura es una ventaja real';
            return 'Para nada: ' . $age_intro . ' para aprender cerrajeria. NALA funciona bien para estudiantes mayores porque las lecciones son online, repetibles y a tu propio ritmo, asi que el entrenamiento encaja alrededor del trabajo, la familia o la jubilacion sin apuro.' . "\n\n" . 'Tu experiencia de vida tambien ayuda: los clientes valoran paciencia, criterio, confianza y profesionalismo, ventajas fuertes en servicios pagados.' . nala_mxchat_signup_skill_investment_phrase($lang) . "\n\n" . 'Pruebalo aqui: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        $age_intro = $age ? $age . ' is a strong age' : 'a mature age is a real advantage';
        return 'Not at all - ' . $age_intro . ' for learning locksmithing. NALA fits older learners because the lessons are online, replayable, and fully self-paced, so training fits around work, family, or retirement without rushing.' . "\n\n" . 'Your life experience helps too: customers value patience, reliability, judgment, and professionalism, which are strong advantages in paid service work.' . nala_mxchat_signup_skill_investment_phrase($lang) . "\n\n" . 'Test the fit here: [Start free lessons / Register](' . $register_link . ').';
    }

    if ($asks_woman_fit) {
        if ($lang === 'es') {
            return 'Si. La cerrajeria no es solo fuerza; premia habilidad, criterio, paciencia, confianza y trato profesional. Para una mujer que quiere una habilidad practica, NALA puede ser una buena forma de aprender desde casa y avanzar a su propio ritmo.' . nala_mxchat_signup_skill_investment_phrase($lang) . "\n\n" . 'Pruebalo gratis aqui: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'Yes. Locksmithing is not just about strength; it rewards skill, judgment, patience, trust, and professional customer service. For a woman who wants a practical trade path, NALA is a strong way to learn from home and move at your own pace.' . nala_mxchat_signup_skill_investment_phrase($lang) . "\n\n" . 'Try it free here: [Start free lessons / Register](' . $register_link . ').';
    }

    if ($asks_beginner_fit) {
        if ($lang === 'es') {
            return 'Si. NALA esta hecho para ayudarte a empezar aunque no tengas experiencia. Las lecciones a tu ritmo y repetibles te ayudan a construir la base antes de avanzar a temas mas practicos.' . nala_mxchat_signup_skill_investment_phrase($lang) . "\n\n" . 'Empieza con las lecciones gratis: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'Yes. NALA is built to help you start even if you have no locksmith experience. The self-paced, replayable lessons help you build the basics before moving into more practical topics.' . nala_mxchat_signup_skill_investment_phrase($lang) . "\n\n" . 'Start with the free lessons: [Start free lessons / Register](' . $register_link . ').';
    }

    if ($asks_online_hands_on) {
        if ($lang === 'es') {
            return 'Si, puedes empezar online. El valor es que puedes pausar, repetir y practicar a tu ritmo; despues combinas las lecciones con practica real usando herramientas y locks permitidos. Practica solo en cerraduras tuyas o con permiso, y revisa las reglas locales antes de ofrecer servicios.' . "\n\n" . 'Empieza gratis y mira si el formato encaja contigo: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'Yes, you can start online. The value is that you can pause, replay, and practice at your pace; then you pair the lessons with real practice using appropriate tools and locks. Practice only on locks you own or have permission to work on, and check local rules before offering services.' . "\n\n" . 'Start free and see if the format fits you: [Start free lessons / Register](' . $register_link . ').';
    }

    if ($asks_license || $asks_criminal) {
        if ($lang === 'es') {
            return 'Los requisitos cambian por estado y ciudad, asi que debes verificar licencias, antecedentes, seguro y reglas comerciales donde planeas trabajar. NALA te da educacion y una ruta de entrenamiento; no puede prometer licencia, aprobacion legal o resultados de antecedentes.' . $personal('terms') . "\n\n" . 'Puedes empezar el entrenamiento gratis mientras revisas tus reglas locales: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'Requirements vary by state and city, so you should check licensing, background, insurance, and business rules where you plan to work. NALA gives you education and a training path; it cannot promise licensing, legal approval, or a background-check outcome.' . $personal('terms') . "\n\n" . 'You can start the training free while you check your local rules: [Start free lessons / Register](' . $register_link . ').';
    }

    if ($asks_trust) {
        if ($lang === 'es') {
            return 'Si, NALA es un programa real de capacitacion en cerrajeria. Tiene mas de 10 anos de experiencia, mas de 20K estudiantes capacitados, lecciones gratis antes de comprar y una muestra de certificado en la pagina principal.' . "\n\n" . 'La forma inteligente de comprobarlo es empezar gratis y ver el entrenamiento: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'Yes, NALA is a real locksmith training program. It has 10+ years of experience, 20K+ students trained, free lessons before purchase, and a sample certificate on the homepage.' . "\n\n" . 'The smart way to check the fit is to start free and see the training: [Start free lessons / Register](' . $register_link . ').';
    }

    if ($asks_business_launch) {
        if ($lang === 'es') {
            return 'Si. NALA puede ayudarte a aprender la habilidad, y Business in a Box esta pensado para el lado de lanzamiento: sitio web, logo personalizado, facturas, resenas, preparacion local/SEO y materiales de inicio. Es para quien quiere clientes, no solo lecciones.' . $personal('bib') . "\n\n" . 'Empieza gratis y luego elige Business in a Box si tu meta es lanzar el servicio: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'Yes. NALA helps you build the skill, and Business in a Box is built for the launch side: website, custom logo, invoices, reviews, local/SEO setup, and launch materials. It is for people who want customers, not just lessons.' . $personal('bib') . "\n\n" . 'Start free, then choose Business in a Box if your goal is to launch the service: [Start free lessons / Register](' . $register_link . ').';
    }

    if ($asks_job_guarantee) {
        if ($lang === 'es') {
            return 'NALA no debe prometer un trabajo o ingresos garantizados. Lo valioso es la preparacion: habilidades practicas, certificacion, practica repetible y Business in a Box si quieres lanzar tu propio servicio. NALA tambien destaca 95% de colocacion laboral como una senal fuerte de enfoque practico.' . "\n\n" . 'Empieza gratis y mira si el camino te motiva: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'NALA should not promise a guaranteed job or guaranteed income. The value is preparation: practical skills, certification, replayable training, and Business in a Box if you want to launch your own service. NALA also highlights 95% job placement as a strong signal that the program is built around practical outcomes.' . "\n\n" . 'Start free and see if the path motivates you: [Start free lessons / Register](' . $register_link . ').';
    }
    ...
    if ($asks_refund) {
        if ($lang === 'es') {
            return 'NALA tiene una politica estricta de no reembolsos una vez que se emite el acceso a la cuenta y al contenido del curso. Por eso la mejor forma de comprar con confianza es empezar gratis, revisar los [Terminos y Condiciones](docs/Terms%20and%20Conditions.pdf), y actualizar cuando estes listo para tomar el entrenamiento en serio.' . nala_mxchat_signup_skill_investment_phrase($lang) . nala_mxchat_signup_klarna_payment_phrase($lang);
        }

        return 'NALA has a strict no-refunds policy once account access to the course platform is issued. That is why the best way to buy with confidence is to start free, review the [Terms and Conditions](docs/Terms%20and%20Conditions.pdf), and upgrade when you are ready to take the training seriously.' . nala_mxchat_signup_skill_investment_phrase($lang) . nala_mxchat_signup_klarna_payment_phrase($lang);
    }

    if ($asks_privacy) {
        if ($lang === 'es') {
            return 'NALA usa datos de cuenta, curso, compra, uso del sitio y cookies para operar y mejorar el programa. Los datos sensibles de tarjeta los manejan procesadores de pago, no el chat, asi que explorar el programa no requiere compartir datos de pago aqui.' . $personal('privacy') . "\n\n" . 'Revisa la [Politica de Privacidad](docs/Privacy%20Policy.pdf) y luego empieza gratis: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'NALA uses account, course, purchase, site-use, and cookie data to operate and improve the program. Sensitive card details are handled by payment processors, not by chat, so exploring the program does not require sharing payment details here.' . $personal('privacy') . "\n\n" . 'Review the [Privacy Policy](docs/Privacy%20Policy.pdf), then start free: [Start free lessons / Register](' . $register_link . ').';
    }

    if ($asks_terms) {
        if ($lang === 'es') {
            return 'Los [Terminos y Condiciones](docs/Terms%20and%20Conditions.pdf) cubren elegibilidad, cuentas, acceso al curso, pagos, no reembolsos, reglas de conducta y limites legales. Tambien debes verificar requisitos de licencia locales.' . $personal('terms') . "\n\n" . 'Leelos como un comprador serio, y luego empieza gratis para ver si NALA es el camino correcto antes de actualizar: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'The [Terms and Conditions](docs/Terms%20and%20Conditions.pdf) cover eligibility, accounts, course access, payments, no refunds, conduct rules, and legal limits. You should also verify local licensing requirements.' . $personal('terms') . "\n\n" . 'Read them like a serious buyer, then start free to see if NALA is the right path before upgrading: [Start free lessons / Register](' . $register_link . ').';
    }

    if ($asks_placement) {
        if ($lang === 'es') {
            return 'NALA destaca un 95% de colocacion laboral, una senal fuerte de que el programa esta orientado a resultados practicos: habilidades reales, certificacion y preparacion para oportunidades.' . "\n\n" . 'Si tu meta es trabajar, enfocate en el camino: aprende las habilidades, practica en serio, aprueba los examenes requeridos y gana confianza para servicios reales.' . $personal('placement') . nala_mxchat_signup_skill_investment_phrase($lang) . "\n\n" . 'Empieza gratis y mira si este camino te motiva: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'NALA highlights 95% job placement, which is a strong signal that the program is built around practical outcomes: real skills, certification, and preparation for opportunities.' . "\n\n" . 'If your goal is work, focus on the path: learn the skills, practice seriously, pass the required exams, and build confidence for real service opportunities.' . $personal('placement') . nala_mxchat_signup_skill_investment_phrase($lang) . "\n\n" . 'Start free and see if this path motivates you: [Start free lessons / Register](' . $register_link . ').';
    }

    if ($asks_origin) {
        if ($lang === 'es') {
            return 'NALA es un programa internacional de capacitacion en cerrajeria que ahora se expande en EE. UU. Tiene mas de 10 anos de experiencia y mas de 20K estudiantes capacitados, asi que no estas probando algo improvisado: estas entrando a un camino de entrenamiento ya probado.' . $personal('identity') . "\n\n" . 'Empieza gratis aqui para habilidades practicas y certificacion: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'NALA is an international locksmith training program now expanding in the U.S. It has 10+ years of experience and 20K+ students trained, so you are not testing something improvised: you are stepping into a proven training path.' . $personal('identity') . "\n\n" . 'Start free here for practical skills and certification: [Start free lessons / Register](' . $register_link . ').';
    }

    if ($asks_identity || $asks_history || $asks_students) {
        if ($lang === 'es') {
            return 'NALA es la North American Locksmith Association. Ofrece capacitacion online de cerrajeria enfocada en habilidades practicas para trabajo automotriz, residencial, comercial, puertas y cajas fuertes. Es una forma emocionante de aprender una habilidad real desde casa, a tu propio ritmo, con un camino claro hacia certificacion y posibles servicios pagados.' . $personal('identity') . "\n\n"
                . '- Experiencia: mas de 10 anos' . "\n"
                . '- Estudiantes capacitados: mas de 20K' . "\n\n"
                . 'El mejor paso es probarlo ahora: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'NALA is the North American Locksmith Association. It offers online locksmith training focused on practical skills for automotive, residential, commercial, door, and safe-related work. It is an exciting way to build a real-world trade skill from home, at your own pace, with a clear path toward certification and potential paid services.' . $personal('identity') . "\n\n"
            . '- Experience: 10+ years' . "\n"
            . '- Students trained: 20K+' . "\n\n"
            . 'The best next step is to try it now: [Start free lessons / Register](' . $register_link . ').';
    }

    if (nala_mxchat_signup_contains_any($normalized, ['business in a box', 'bib', 'business-in-a-box', 'negocio en una caja'])) {
        if ($lang === 'es') {
            return 'Business in a Box es el lado de lanzamiento del negocio: te guia paso a paso con sitio web gratis, logo personalizado gratis, generador de facturas gratis, pagos, SEO/local, resenas y materiales de lanzamiento. Tambien incluye un set de lock pick con la compra. Ayuda a empezar, pero no reemplaza una caja profesional completa.' . $personal('bib') . "\n\n" . 'Si quieres clientes y no solo lecciones, este es el upgrade mas completo para convertir el entrenamiento en un negocio real.' . "\n\n" . 'Empieza gratis aqui: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'Business in a Box is the business-launch side of NALA. It guides you step by step with a free website, free custom logo, free invoice creator, payment setup, SEO/local setup, reviews, launch materials, and a lock pick tool set with purchase. It helps you start, but it is not a complete professional toolbox.' . $personal('bib') . "\n\n" . 'If you want customers, not just lessons, this is the most complete upgrade for turning training into a real business.' . "\n\n" . 'Start free here: [Start free lessons / Register](' . $register_link . ').';
    }

    if (nala_mxchat_signup_contains_any($normalized, ['certification', 'certificate', 'certified', 'certificacion', 'certificado'])) {
        if ($lang === 'es') {
            return 'Obtienes la certificacion de NALA completando el camino de entrenamiento requerido y el paso final de certificacion en tu panel. La certificacion te da una meta clara y una forma de mostrar que tomaste el entrenamiento en serio.' . $personal('certification') . nala_mxchat_signup_skill_investment_phrase($lang) . "\n\n" . 'Empieza gratis y mira el camino: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'Earn NALA certification by completing the required training path and final certification step in your dashboard. Certification gives you a clear goal and a structured way to show you took the training seriously.' . $personal('certification') . nala_mxchat_signup_skill_investment_phrase($lang) . "\n\n" . 'Start free and see the path: [Start free lessons / Register](' . $register_link . ').';
    }
    ...
    if (nala_mxchat_signup_contains_any($normalized, ['support', 'contact', 'help with my account', 'login problem', 'cant log in', 'cannot log in', 'soporte', 'contacto', 'problema con mi cuenta', 'no puedo entrar'])) {
        if ($lang === 'es') {
            return 'Para ayuda general o preguntas sobre tu cuenta, contacta ' . $support_link . '. Si ya tienes cuenta, usa el enlace de iniciar sesion en el sitio.' . "\n\n" . 'Si aun estas explorando NALA, no esperes por una pregunta general. Empieza gratis y mira el entrenamiento mientras tanto: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        }

        return 'For general help or account questions, contact ' . $support_link . '. If you already have an account, use the login link on the site.' . "\n\n" . 'If you are still exploring NALA, do not wait on a general question. Start free and see the training while you decide: [Start free lessons / Register](' . $register_link . ').';
    }
...
function nala_mxchat_signup_klarna_payment_phrase(string $lang): string
{
    ...
    if ($lang === 'es') {
        if ($premium) {
            return "\n\n" . 'Cuando estes listo, Klarna muestra Premium como ' . $premium['monthly'] . ' por mes durante 24 meses en checkout cuando esta disponible y aprobado.';
        }

        return "\n\n" . 'Cuando esta disponible y aprobado, Klarna divide el pago durante 24 meses en checkout.';
    }

    if ($premium) {
        return "\n\n" . 'When you are ready, Klarna shows Premium as ' . $premium['monthly'] . ' per month for 24 months at checkout when available and approved.';
    }

    return "\n\n" . 'When available and approved, Klarna spreads the payment over 24 months at checkout.';
}

function nala_mxchat_signup_skill_investment_phrase(string $lang): string
{
    if ($lang === 'es') {
        return "\n\n" . 'El trabajo pagado de cerrajeria es el camino que ayuda a recuperar la inversion con el tiempo mientras tus habilidades crecen.';
    }

    return "\n\n" . 'Paid locksmith work is the path that helps the training pay for itself over time as your skills grow.';
}
...
function nala_mxchat_signup_pricing_answer(string $lang, string $register_link, array $personal_context = []): string
{
    ...
    if ($lang === 'es') {
        ...
        $lines[] = '';
        $lines[] = 'Premium y Business in a Box estan pensados para acercarte a servicios pagados con entrenamiento, herramientas y sistema de lanzamiento.';
        $lines[] = 'Empieza con las lecciones gratis y actualiza cuando veas que NALA es el camino correcto para ti: [Empezar lecciones gratis / Registrarte](' . $register_link . ').';
        return implode("\n", $lines);
    }
    ...
    $lines[] = '';
    $lines[] = 'Premium and Business in a Box are built to move you toward paid services with training, tools, and a launch system.';
    $lines[] = 'Start with the free lessons and upgrade once you see NALA is the right path for you: [Start free lessons / Register](' . $register_link . ').';
    return implode("\n", $lines);
}
...
function nala_mxchat_signup_text(string $key, array $state): string
{
    ...
    $strings = [
        'en' => [
            'ask_name' => 'Great - I will help create your free NALA account so the lessons are ready to start. Please send your full name.',
            ...
            'success' => 'After registration is completed, we email the next steps so you set your password and start the free lessons.' . "\n\n" . 'The [Register page](' . $register_link . ') is also available.',
            ...
            'cancelled' => 'No problem. I stopped the chat signup.' . "\n\n" . 'Use the [Register page](' . $register_link . ') whenever you are ready.',
        ],
        'es' => [
            'ask_name' => 'Excelente - voy a ayudar a crear tu cuenta gratis de NALA para que las lecciones queden listas. Envia tu nombre completo.',
            ...
            'success' => 'Despues de completar el registro, enviamos los siguientes pasos por correo para que configures tu contrasena y empieces las lecciones gratis.' . "\n\n" . 'La [pagina de registro](' . $register_link . ') tambien esta disponible.',
            ...
            'cancelled' => 'Sin problema. Detuve el registro por chat.' . "\n\n" . 'Usa la [pagina de registro](' . $register_link . ') cuando estes listo.',
        ],
    ];
