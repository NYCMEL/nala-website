...
class ClientProfile {
	...
	try {
		...
		this.data.business.logoKind = stored.logoKind || 'artwork'
	...
	window.addEventListener('message', (event) => {
	    ...
	    this.data.business.logoKind = event.data.payload.logoKind || 'artwork'
	...
	try {
	    ...
	    const setup = JSON.parse(localStorage.getItem('nala_biab_setup_state_v1') || 'null')
	    const paletteId = setup && setup.values && setup.values.palette
	    const palette = this.resolveBrandTheme(paletteId)
	    if (palette) this.data.business.theme = palette
	} catch (err) {
	    wc.warn('[client] Could not apply stored BIAB theme', err)
	}
    }
	...
	const logo = (changes.clientLogo && changes.clientLogo.startsWith('data:'))
	    ? changes.clientLogo
	    : d.business.logo
	const theme = d.business.theme ? JSON.stringify(d.business.theme) : 'null'
	const logoKind = d.business.logoKind || ((String(logo).indexOf('data:image/svg') === 0 || String(logo).indexOf('logo') !== -1) ? 'artwork' : 'photo')
	...
	const logoSrc = this.data.business.logo || ""
	const isLogoArtwork = this.data.business.logoKind === 'artwork' || logoSrc.indexOf('data:image/svg') === 0 || logoSrc.indexOf('logo') !== -1 || logoSrc.indexOf('nalaBiab') !== -1
	logo.classList.toggle('client-logo--artwork', isLogoArtwork)
	...
	// Tab navigation — push state so browser back/forward works
	...
	tabs.forEach((tab) => {
		...
		// Push tab id into URL hash so back/forward restores it
		const tabId = e.target.dataset.tab || e.target.textContent.trim()
		if (tabId) {
		    history.pushState({ tab: tabId }, '', '#tab=' + encodeURIComponent(tabId))
		}
	    })
	})
	...
	;(function() {
	    ...
	    const hashTab = m ? decodeURIComponent(m[1]) : null
	    if (hashTab) {
		tabs.forEach(t => {
		    const id = t.dataset.tab || t.textContent.trim()
		    t.classList.toggle("active", id === hashTab)
		})
	    }
	})()
	...
	window.addEventListener("popstate", function(e) {
	    const tabId = (e.state && e.state.tab) || null
		...
		allTabs.forEach(t => {
		    const id = t.dataset.tab || t.textContent.trim()
		    if (id === tabId) t.classList.add("active")
		})
	    } else {
		allTabs[0].classList.add("active")
	    }
	    wc.log("[client] popstate → tab:", tabId || "first")
	})
