...
class Pages extends HTMLElement {
    ...
    _process(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            tstr += `<div class="wc-${this.cname}__page ${data[i].page}" wc-pages-id="${data[i].page}"></div>`;
        }
    ...
    _dev(page) {
        ...
        for (var i = 0; i < this.data.length; i++) {
            str += `<button type="button" class="btn btn-sm btn-success wc-pages-btn-tester" style="font-size:11px;border-radius:0">${this.data[i].page}</button>`;
        }
        ...
        $(`.wc-pages-btn-tester`).on("click", function () {
            self.show($(this).text());
        });
    ...
    new(values) {
        ...
        $(`#${this.id} .btn-group-tester`).append(
            `<button type="button" class="btn btn-sm btn-danger wc-pages-btn-tester" style="font-size:11px;border-radius:0">${values.page}</button>`
        );

        $(`#${this.id} .wc-pages-btn-tester`).unbind().on("click", function () {
            self.show($(this).text());
        });
...
window.customElements.define('wc-pages', Pages);
