        ...
        if (Array.isArray(config.actions)) {
            config.actions.forEach(action => {
                ...
                btn.addEventListener("click", () => {
                    ...
                    const eventName = action.event || `dialog.${action.type}`;

                    // REQUIRED: log before publishing
                    console.log("[_dialog] Publishing:", eventName);

                    // Use your wc.publish from _wc.js
                    wc.publish(eventName, {
                        type: action.type,
                        source: "_dialog"
                    });
