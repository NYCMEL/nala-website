...
class MTKHierarchy {
	...
	if (typeof wc !== 'undefined') {
	    wc.log("MTKHierarchy: Loading mtk-quiz component", quiz.title);
	}
	
	// Clear RHS and load mtk-quiz component
	this.elements.rhs.innerHTML = '<mtk-quiz class="mtk-quiz-component"></mtk-quiz>';
	
	// Pass quiz data to the mtk-quiz component
	// The component should be listening for this event or have a method to receive data
	if (typeof wc !== 'undefined') {
	    wc.publish('mtk-quiz:load', {
		quiz: quiz,
		moduleId: moduleId,
		timestamp: new Date().toISOString()
	    });
	...
	// Store quiz data for component access
	if (window.MTKQuiz) {
	    // If mtk-quiz component has a load method
	    const quizComponent = this.elements.rhs.querySelector('.mtk-quiz-component');
	    if (quizComponent && typeof quizComponent.loadQuiz === 'function') {
		quizComponent.loadQuiz(quiz, moduleId);
	    }
	}
	
	if (typeof wc !== 'undefined') {
	    wc.log("MTKHierarchy: mtk-quiz component loaded");
	}
    }
