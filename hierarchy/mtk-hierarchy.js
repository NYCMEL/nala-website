...
class MTKHierarchy {
    ...
    /**
     * Handle quiz start - public method called externally after quiz completion
     */
	...
	if (videos.length === 0 && photos.length === 0) {
	    contentHTML += '<p style="color: var(--mtk-text-secondary);">No accessible content available for this resource.</p>';
	}
    ...
    /**
     * Get all resources for the current lesson (used by displayResource)
     */
	...
	for (const course of this.config) {
		...
		for (const lesson of module.lessons) {
			...
			this.activeResource = resource.id;
...
// Guard: if window.MTKHierarchy already exists, skip creation entirely
// ─────────────────────────────────────────────────────────────────────────────
if (wc.isLocal) {
	...
	if (!window.MTKHierarchy) {
	    ...
	    wc.log("MTKHierarchy: Already initialised, skipping creation");
	}
    } else {
...
function subscribeToEvents() {
	...
	if (lessonNo !== null && current !== null && lessonNo > current) {
	    wc.session.user.current_lesson = lessonNo;
	    wc.log("MTKHierarchy: Updated current_lesson to", lessonNo);
	}
    });
}
