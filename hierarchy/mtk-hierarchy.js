...
class MTKHierarchy {
  ...
  displayQuiz(quiz, moduleId) {
    ...
    if (typeof wc !== 'undefined') {
      wc.log("MTKHierarchy: Showing quiz via mtk_pager", quiz.title);
    }
    
    // Call mtk_pager to show quiz page
    if (typeof mtk_pager !== 'undefined' && mtk_pager.show) {
      mtk_pager.show("quiz");
    } else {
      if (typeof wc !== 'undefined') {
        wc.error("MTKHierarchy: mtk_pager is not defined or does not have show method");
      }
    }
  }
