...
class MTKHierarchy {
	...
	this.config.forEach((course, cIdx) => {
		...
		if (!module.id) {
		    wc.warn(`MTKHierarchy: Module missing ID, generating fallback: module-${cIdx}-${mIdx}`);
		    module.id = `module-${cIdx}-${mIdx}`;
		}
		...
		module.lessons.forEach((lesson, lIdx) => {
		    // Only add ID if missing - otherwise use from JSON
		    if (!lesson.id) {
			wc.warn(`MTKHierarchy: Lesson missing ID, generating fallback: lesson-${cIdx}-${mIdx}-${lIdx}`);
			lesson.id = `lesson-${cIdx}-${mIdx}-${lIdx}`;
		    }
			...
			// Only add ID if missing - otherwise use from JSON
			if (!resource.id) {
			    wc.warn(`MTKHierarchy: Resource missing ID, generating fallback: resource-${cIdx}-${mIdx}-${lIdx}-${rIdx}`);
			    resource.id = `resource-${cIdx}-${mIdx}-${lIdx}-${rIdx}`;
			}
		...
		if (module.quiz && !module.quiz.id) {
		    wc.warn(`MTKHierarchy: Quiz missing ID, generating fallback: quiz-${module.id}`);
		    module.quiz.id = `quiz-${module.id}`;
		}
	    });
	});
    }
