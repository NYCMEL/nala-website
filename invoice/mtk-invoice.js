class MtkInvoice {
  ...
  onMessage(topic, data) {
    ...
    if (topic === "4-mtk-invoice:print") this._print();
        ...
        <div class="mtk-invoice__card">
          <header class="mtk-invoice__header">
            ...
            <button class="mtk-invoice__print-btn" type="button" data-action="print">
              ...
              <span>${this._escape(this.labels.printButton || "Print / Save PDF")}</span>
            </button>
          </header>
          ...
          <form class="mtk-invoice__form" novalidate>
            <section class="mtk-invoice__section" aria-label="Business and invoice information">
              <div class="mtk-invoice__grid">${this._renderFields("business")}</div>
            </section>
            <section class="mtk-invoice__section" aria-labelledby="mtk-invoice-customer-heading">
              ...
              <div class="mtk-invoice__grid">${this._renderFields("customer")}</div>
            </section>
            <section class="mtk-invoice__section" aria-labelledby="mtk-invoice-service-heading">
              ...
              <div class="mtk-invoice__grid mtk-invoice__grid--three">${this._renderFields("service")}</div>
            </section>
            <section class="mtk-invoice__section" aria-labelledby="mtk-invoice-totals-heading">
              ...
              <div class="mtk-invoice__totals" aria-live="polite">
                <div class="mtk-invoice__total-row"><span>Subtotal</span><strong data-total="subtotal">$0.00</strong></div>
                <div class="mtk-invoice__total-row"><span>Tax</span><strong data-total="tax">$0.00</strong></div>
                <div class="mtk-invoice__total-row mtk-invoice__total-row--grand"><span>Total</span><strong data-total="total">$0.00</strong></div>
              </div>
            </section>
  ...
  _renderFields(groupName) {
    const fields = this.config.fields && Array.isArray(this.config.fields[groupName]) ? this.config.fields[groupName] : [];
  ...
  _renderField(field) {
    ...
    if (field.type === "select") {
      return `<div class="${fieldClass}"><label for="mtk-invoice-${this._escape(field.id)}">${this._escape(field.label)}</label><select id="mtk-invoice-${this._escape(field.id)}" name="${this._escape(field.id)}" data-field="${this._escape(field.id)}"${required}>${(field.options || []).map((option) => `<option value="${this._escape(option)}"${value === option ? " selected" : ""}>${this._escape(option)}</option>`).join("")}</select></div>`;
    }
    if (field.type === "textarea") {
      return `<div class="${fieldClass}"><label for="mtk-invoice-${this._escape(field.id)}">${this._escape(field.label)}</label><textarea id="mtk-invoice-${this._escape(field.id)}" name="${this._escape(field.id)}" data-field="${this._escape(field.id)}" placeholder="${this._escape(field.placeholder || "")}"${required}>${this._escape(value)}</textarea></div>`;
    }
    return `<div class="${fieldClass}"><label for="mtk-invoice-${this._escape(field.id)}">${this._escape(field.label)}</label><input id="mtk-invoice-${this._escape(field.id)}" name="${this._escape(field.id)}" data-field="${this._escape(field.id)}" type="${this._escape(field.type || "text")}" value="${this._escape(value)}"${required}></div>`;
  }
  ...
  _bind() {
    this.root.addEventListener("input", (event) => {
      ...
      this._updateTotals();
      this._publish(this.events.publish.change || "mtk-invoice:change", { values: this._getValues(), totals: this._calculateTotals() });
    });
  ...
  _print() {
    this._publish(this.events.publish.print || "mtk-invoice:print", { values: this._getValues(), totals: this._calculateTotals() });
    window.print();
  }
  ...
  _save() {
    ...
    this._publish(this.events.publish.save || "mtk-invoice:save", { values: this._getValues(), totals: this._calculateTotals() });
  }
  ...
  _updateTotals() {
    ...
    Object.keys(totals).forEach((key) => {
      ...
      if (target) target.textContent = this._formatCurrency(totals[key]);
    });
  }
  ...
  _getValues() { return Object.assign({}, this.values); }
  _number(value) { const parsed = Number(value); return Number.isFinite(parsed) ? parsed : 0; }
  _formatCurrency(value) { return "$" + (Number(value) || 0).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }); }
  ...
  static initWhenReady() {
    const start = () => {
      ...
      if (!root || root.dataset.mtkInvoiceReady === "true") return false;
